...
class YandexDiskRestClient:
    ...
    def get_disk_metadata(self):
        ...
        r = requests.get(url, headers=self.base_headers, verify=False)
        self._check_code(r)
    ...
    def get_content_of_folder(self, path_to_folder):
        ...
        payload = {'path': path_to_folder, 'limit': '1000000'}
        r = requests.get(url, headers=self.base_headers, params=payload, verify=False)
        self._check_code(r)
        json_dict = r.json()
    ...
    def create_folder(self, path_to_folder):
        ...
        r = requests.put(url, headers=self.base_headers, params=payload, verify=False)
        self._check_code(r)
    ...
    def remove_folder_or_file(self, path):
        ...
        payload = {'path': path}
        r = requests.delete(url, headers=self.base_headers, params=payload, verify=False)
        self._check_code(r)
    ...
    def copy_folder_or_file(self, path_from, path_to):
        ...
        r = requests.post(url, headers=self.base_headers, params=payload, verify=False)
        self._check_code(r)
    ...
    def get_list_last_files(self):
        """
        :return: List of all files
        """
        url = self._base_url + "/resources/last-uploaded"

        r = requests.get(url, headers=self.base_headers, params = {'limit': '1000'}, verify=False)
        self._check_code(r)
    ...
    def upload_file(self, path_from, path_to):
        ...
        with open(path_from, 'rb') as fh:
            ...
            r2 = requests.put(upload_link, headers=self.base_headers, files=files, verify=False)
            self._check_code(r2)
